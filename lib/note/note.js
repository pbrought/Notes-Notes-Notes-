...
exports.grabNotes = function(id, callback) {
	dbHelp.dbConn( function ( err, db ){
		var collection = db.collection( 'notes' );
		if(err)
			callback(err);
		else{
			/*collection.find({title : "title1"})
	...
	collection.findOne({notebookID: id }, function(err, item) {
				if(err)
					callback(err);
				else{
					callback(undefined, item);			
	...
	dbHelp.dbConn( function( err, db ) {
		...
		notes
			.find( { $or : [
				...
				{notebookID : { $regex : str, $options : 'i' } }
			] }, { title : 1, tags : 1, notebookID : 1, sharedWith : 1 } )
			.toArray( function( err, docs ) {
				if ( err ) console.dir( err );
				docs.forEach(function (value, index, ar) {
					ar[index].type = 'note';
