...
exports.findUser = function( username, callback) {
    MongoClient.connect('mongodb://127.0.0.1:27017/notes', function (err, db) {
        ...
        userz = db.collection('users');
        callback(userz.find({username: username}).toArray(function (err, items) {
            if (err)
                callback(-1);
...
exports.authent = function( username, password, callback ) {
    MongoClient.connect('mongodb://127.0.0.1:27017/notes', function (err, db) {
        ...
        callback(userz.find({username: username, password: password}).toArray(function (err, items) {
            if (err)
                callback(-1);
            else if(!items.length){
                callback(0);
        ...
        callback(userz.find( { username: username, password: password }, {userid: 1, _id: 0} ));
    })
};